...
class BitmapHunter implements Runnable {
    ...
    Bitmap result;
    Future<?> future;
    NativePicasso.LoadedFrom loadedFrom;
    Exception exception;
    int exifOrientation; // Determined during decoding of original resource.
    int retryCount;
    NativePicasso.Priority priority;

    BitmapHunter(NativePicasso picasso, Dispatcher dispatcher, Cache cache, Stats stats, Action action,
    ...
    static Bitmap decodeStream(InputStream inputStream, Request request) throws IOException {
        ...
        boolean isWebPFile = Utils.isWebPFile(inputStream);
        ...
        if (isWebPFile || isPurgeable) {
            byte[] bytes = Utils.readByteArray(inputStream);
            if (calculateSize) {
    ...
    Bitmap hunt() throws IOException {
        ...
        if (MemoryPolicy.shouldReadFromMemoryCache(memoryPolicy)) {
            ...
            if (bitmap != null) {
                ...
                loadedFrom = NativePicasso.LoadedFrom.MEMORY;
                if (picasso.loggingEnabled) {
    ...
    void attach(Action action) {
        ...
        NativePicasso.Priority actionPriority = action.getPriority();
    ...
    private NativePicasso.Priority computeNewPriority() {
        NativePicasso.Priority newPriority = NativePicasso.Priority.LOW;
        ...
        if (hasMultiple) {
            ...
            for (int i = 0, n = actions.size(); i < n; i++) {
                NativePicasso.Priority actionPriority = actions.get(i).getPriority();
    ...
    NativePicasso.LoadedFrom getLoadedFrom() {
        return loadedFrom;
    }

    NativePicasso.Priority getPriority() {
        return priority;
    }
    ...
    static BitmapHunter forRequest(NativePicasso picasso, Dispatcher dispatcher, Cache cache, Stats stats,
                                   Action action) {
