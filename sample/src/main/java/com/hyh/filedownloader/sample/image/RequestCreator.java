...
public class RequestCreator {
    ...
    private final NativePicasso picasso;
    ...
    RequestCreator(NativePicasso picasso, Uri uri, int resourceId) {
        if (picasso.shutdown) {
            throw new IllegalStateException(
                    "NativePicasso instance already shut down. Cannot submit new requests.");
        }
     ...
     * <p>
     * By default, NativePicasso will either set a supplied placeholder or clear the target
     ...
     * <p>
     * <strong>WARNING:</strong>: NativePicasso will keep a reference to the tag for
     * as long as this tag is paused and/or has active requests. Look out for
     * potential leaks.
     *
     * @see NativePicasso#cancelTag(Object)
     * @see NativePicasso#pauseTag(Object)
     * @see NativePicasso#resumeTag(Object)
     */
     ...
     * By default, all requests have {@link NativePicasso.Priority#NORMAL} priority, except for
     * {@link #fetch()} requests, which have {@link com.yly.lib.picasso.image.NativePicasso.Priority#LOW} priority by default.
     */
    public RequestCreator priority(NativePicasso.Priority priority) {
    ...
    public void fetch(Callback callback) {
        ...
        if (data.hasImage()) {
            ...
            if (!data.hasPriority()) {
                data.priority(NativePicasso.Priority.LOW);
            }
            ...
            if (MemoryPolicy.shouldReadFromMemoryCache(memoryPolicy)) {
                ...
                if (bitmap != null) {
                    if (picasso.loggingEnabled) {
                        Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, request.plainId(), "from " + NativePicasso.LoadedFrom.MEMORY);
                    }
                    if (callback != null) {
                        callback.onSuccess();
                    }
    ...
    public void into(Target target) {
        ...
        if (MemoryPolicy.shouldReadFromMemoryCache(memoryPolicy)) {
            ...
            if (bitmap != null) {
                picasso.cancelRequest(target);
                target.onBitmapLoaded(bitmap, NativePicasso.LoadedFrom.MEMORY);
                return;
            }
        }
     ...
     */
    public void into(RemoteViews remoteViews, int viewId, int notificationId,
                     Notification notification, String notificationTag, Callback callback) {
        long started = System.nanoTime();
     ...
     */
    public void into(RemoteViews remoteViews, int viewId,
                     int[] appWidgetIds) {
     ...
     * {@link NativePicasso#cancelRequest(ImageView)} call to prevent temporary leaking.
     */
    public void into(ImageView target, Callback callback) {
        ...
        if (MemoryPolicy.shouldReadFromMemoryCache(memoryPolicy)) {
            ...
            if (bitmap != null) {
                picasso.cancelRequest(target);
                PicassoDrawable.setBitmap(target, picasso.context, bitmap, NativePicasso.LoadedFrom.MEMORY, noFade, picasso.indicatorsEnabled);
                if (picasso.loggingEnabled) {
                    Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, request.plainId(), "from " + NativePicasso.LoadedFrom.MEMORY);
                }
                if (callback != null) {
    ...
    private void performRemoteViewInto(RemoteViewsAction action) {
        if (MemoryPolicy.shouldReadFromMemoryCache(memoryPolicy)) {
            ...
            if (bitmap != null) {
                action.complete(bitmap, NativePicasso.LoadedFrom.MEMORY);
                return;
            }
        }
