...
public class ProgressButton extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        if (attrs != null) {
            ...
            mTextSize = typedArray.getDimension(R.styleable.ProgressButton_textSize, DisplayUtil.dip2px(context, 14));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (foregroundWidth > 0 && foregroundWidth <= mWidth - corner) {
            float progressHeight = computeProgressHeighetWithCorner(foregroundWidth, corner);
    ...
    private void drawText(Canvas canvas, int foregroundWidth) {
        ...
        if (foregroundWidth <= mTextStartX) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                canvas.clipRect(foregroundWidth, 0, mTextStartX + mTextWidth, mHeight);
            } else {
    ...
    private float computeProgressHeighetWithCorner(int foregroundWidth, float corner) {
