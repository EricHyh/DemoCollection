 ...
 */
class BlockPackageManagerStaticProxy extends PackageManager {

    private PackageManager mBase;

    BlockPackageManagerStaticProxy(PackageManager packageManager) {
        this.mBase = packageManager;
    }

    @Override
    public PackageInfo getPackageInfo(String packageName, int flags) throws NameNotFoundException {
        PackageInfo packageInfo = mBase.getPackageInfo(packageName, flags);
        if (packageInfo != null && TextUtils.equals(packageName, BlockApplication.hostApplicationContext.getPackageName())) {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags) throws NameNotFoundException {
        ApplicationInfo applicationInfo = mBase.getApplicationInfo(packageName, flags);
        if (applicationInfo != null && TextUtils.equals(packageName, BlockApplication.hostApplicationContext.getPackageName())) {
            PackageManagerHelper packageManagerHelper = new PackageManagerHelper();
    ...
    public ResolveInfo resolveActivity(Intent intent, int flags) {
        ResolveInfo resolveInfo = mBase.resolveActivity(intent, flags);
        //如果找不到再进行加工，如果已经找到，没必要再处理
        if (resolveInfo == null) {
            if (!PackageManagerHelper.checkIntentComponent(intent)) {
                return null;
            }
            PackageManagerHelper packageManagerHelper = new PackageManagerHelper();
            return packageManagerHelper.getBlockResolveActivity(intent, flags);
        }
        return resolveInfo;
    }
    ...
    public List<ResolveInfo> queryIntentActivities(Intent intent, int flags) {
        List<ResolveInfo> resolveInfos = mBase.queryIntentActivities(intent, flags);
        if (!PackageManagerHelper.checkIntentComponent(intent)) {
            return resolveInfos;
        } else {
            PackageManagerHelper packageManagerHelper = new PackageManagerHelper();
            resolveInfos = packageManagerHelper.addBlockIntentActivities(resolveInfos, intent, flags);
        }
        return resolveInfos;
    }
    ...
    public ResolveInfo resolveService(Intent intent, int flags) {
        ResolveInfo resolveInfo = mBase.resolveService(intent, flags);
        //如果找不到再进行加工，如果已经找到，没必要再处理
        if (resolveInfo == null) {
            if (!PackageManagerHelper.checkIntentComponent(intent)) {
                return null;
            ...
            PackageManagerHelper packageManagerHelper = new PackageManagerHelper();
            return packageManagerHelper.getBlockResolveService(intent, flags);
        }
        return resolveInfo;
    }
    ...
    public List<ResolveInfo> queryIntentServices(Intent intent, int flags) {
        List<ResolveInfo> resolveInfos = mBase.queryIntentServices(intent, flags);
        if (!PackageManagerHelper.checkIntentComponent(intent)) {
            return resolveInfos;
        } else {
            PackageManagerHelper packageManagerHelper = new PackageManagerHelper();
            resolveInfos = packageManagerHelper.addBlockIntentServices(resolveInfos, intent, flags);
        }
        return resolveInfos;
    }
